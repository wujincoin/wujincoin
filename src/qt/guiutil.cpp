...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("WuJinCoin"))
        return false;
...
{
    // Convert WuJinCoin:// to WuJinCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("WuJinCoin://"))
    {
        uri.replace(0, 12, "WuJinCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WuJinCoin.lnk";
}
...
{
    return GetAutostartDir() / "WuJinCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("WuJinCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  WuJinCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("WuJinCoin-Qt"));
